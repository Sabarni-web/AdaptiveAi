...
Student Answer: ${studentAnswer}`;
  ...
  async detectAIGeneratedText(text: string): Promise<boolean> {
    try {
      const detectionApiKey = process.env.GEMINI_DETECTION_API_KEY || process.env.GEMINI_API_KEY;
      if (!detectionApiKey) return false;
...
Return ONLY a valid JSON object with a single boolean field "isAIGenerated".
