...
Student Answer: ${studentAnswer}`;
  ...
  async detectAIGeneratedText(text: string): Promise<{isAIGenerated: boolean, aiPercentage: number}> {
    try {
      const detectionApiKey = process.env.GEMINI_DETECTION_API_KEY || process.env.GEMINI_API_KEY;
      if (!detectionApiKey) return { isAIGenerated: false, aiPercentage: 0 };
      const detectionGenAI = new GoogleGenerativeAI(detectionApiKey);
      const detectionModel = detectionGenAI.getGenerativeModel({ model: 'gemini-flash-latest' });
      const prompt = `Analyze the following text and determine if it is highly likely to be generated by an AI (LLM). 
Return ONLY a valid JSON object with two fields: 
- "isAIGenerated" (boolean) 
- "aiPercentage" (number between 0 and 100, representing the estimated probability that the text is AI-generated).
Text to analyze: "${text}"`;
      ...
      if (match) {
        ...
        return {
          ...
          aiPercentage: typeof json.aiPercentage === 'number' ? json.aiPercentage : (json.isAIGenerated ? 90 : 10)
        };
      }
      return { isAIGenerated: false, aiPercentage: 0 };
    } catch (error) {
      logger.error('[AI_SERVICE] AI detection failed: ' + error);
      return { isAIGenerated: false, aiPercentage: 0 };
    }
  }
}
